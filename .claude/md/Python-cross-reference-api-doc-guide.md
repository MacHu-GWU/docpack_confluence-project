...
### docpack_confluence/auth/users.py
...
### docpack_confluence/auth/permissions.py
...
### docpack_confluence/database/models.py
...
### docpack_confluence/database/connection.py
...
def process_user():
    ...
    Creates a :class:`~docpack_confluence.auth.users.User` instance and validates
    permissions through :class:`~docpack_confluence.auth.permissions.Role`.
    """
    pass
```
...
def update_profile():
    ...
    Uses :meth:`User.get_profile <docpack_confluence.auth.users.User.get_profile>` 
...
def login_user():
    ...
    Calls :func:`~docpack_confluence.auth.users.authenticate` to verify credentials
    and :func:`~docpack_confluence.auth.permissions.check_permission` for authorization.
    """
    pass
```
...
def setup_auth():
    ...
    Configures components from :mod:`docpack_confluence.auth.users` and 
    :mod:`docpack_confluence.auth.permissions` modules.
    """
    pass
```

**Rendered as**: Configures components from `docpack_confluence.auth.users` and `docpack_confluence.auth.permissions` modules.

### 2. Referencing Elements Within the Same Module

Use short paths when referencing elements in the same Python file:

#### Example: docpack_confluence/auth/users.py
...
:ref:`getting-started` guide, then review the :class:`~docpack_confluence.database.models.BaseModel`
class documentation.

For connection management, see :func:`~docpack_confluence.database.connection.connect`
...
:func:`~docpack_confluence.database.connection.connect`.
...
class BaseModel:
    ...
    through :func:`~docpack_confluence.database.connection.connect`.
    
    Integration with authentication is handled through 
    :class:`~docpack_confluence.auth.users.User` instances. User permissions are
    validated using :func:`~docpack_confluence.auth.permissions.check_permission`.
    ...
    def save(self) -> None:
        ...
        Uses the connection established by :func:`~docpack_confluence.database.connection.connect`.
        Requires authentication through :func:`~docpack_confluence.auth.users.authenticate`.
        """
        pass
    ...
    def find_by_id(self, record_id: int) -> Optional[dict]:
        ...
        .. seealso::
            ...
            :func:`~docpack_confluence.database.connection.Database.query` for custom queries.
        """
        pass
```
...
class UserManager:
    ...
    **Core Components:**
    ...
    - User creation: :class:`~docpack_confluence.auth.users.User`
    - Authentication: :func:`~docpack_confluence.auth.users.authenticate`
    - Authorization: :func:`~docpack_confluence.auth.permissions.check_permission`
    - Data persistence: :class:`~docpack_confluence.database.models.BaseModel`
...
:class:`docpack_confluence.auth.users.User`
